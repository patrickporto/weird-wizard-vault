<script lang="ts">
    ...
    import { activeTab, modalState } from '$lib/stores/characterStore';
    import { Sword, Bomb, Wand2, Zap, Backpack } from 'lucide-svelte';
    import { fade } from 'svelte/transition';
    import { ITEM_TYPES } from '$lib/constants';

    const { useConsumable } = sotdlCharacterActions;

    // Filter equipment for weapons and explosives.
    // Types need to match what's in ITEM_TYPES or what's expected for items.
    let actions = $derived($sotdlCharacter.equipment.filter(i => (i.type === ITEM_TYPES.WEAPON && i.equippedState) || i.type === ITEM_TYPES.EXPLOSIVE));
    let hasSpells = $derived($sotdlCharacter.spells.length > 0);
    let hasTalents = $derived($sotdlCharacter.talents.length > 0);
                ...
                <div class="flex-1">
                    ...
                    {#if item.traits}
                        <div class="flex flex-wrap gap-1 mt-1">
                            {#each item.traits.split(',').map((t: string) => t.trim()).filter((t: string) => t) as trait}
