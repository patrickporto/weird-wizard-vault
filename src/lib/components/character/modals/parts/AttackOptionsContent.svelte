<script lang="ts">
    ...
    import { Dices, Wand2, Sword } from 'lucide-svelte';
    import { modalState, character } from '$lib/stores/characterStore';
    ...
    function selectRoll(type: 'attack' | 'damage') {
        ...
        let rollDataType = '';
        if (type === 'attack') rollDataType = 'weapon_attack';
        else if (type === 'damage') rollDataType = 'weapon_damage';
        ...
        <div class="space-y-6 pt-2">
            ...
            <div class="relative overflow-hidden bg-slate-900 border border-slate-800 rounded-2xl p-6 group transition-all hover:border-indigo-500/30">
                ...
                <div class="flex flex-wrap gap-2 mb-4 relative z-10">
                    ...
                    {#if data.traits}
                        {#each data.traits.split(',').map((t: string) => t.trim()).filter((t: string) => t) as trait}
