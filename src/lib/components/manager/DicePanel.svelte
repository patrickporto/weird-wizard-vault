...
<div class="bg-slate-900 border border-slate-800 rounded-xl p-4 sticky top-4 shadow-xl">
    <h3 class="font-bold text-slate-400 text-xs mb-3 flex items-center gap-2 uppercase tracking-widest"><Dices size={14}/> Rolagens Rápidas</h3>
    <div class="grid grid-cols-2 gap-2">
        ...
        <button 
            ...
            class="bg-slate-800 hover:bg-indigo-600 text-white font-bold py-3 rounded-lg flex items-center justify-center gap-2 transition-all active:scale-95 border border-white/5 text-sm"
            aria-label="Rolar 3d6"
        >
            3d6
        </button>
    </div>
    <p class="text-[10px] text-slate-500 mt-3 text-center italic tracking-tight">O resultado será registrado no histórico.</p>
</div>
