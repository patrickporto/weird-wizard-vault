<script lang="ts">
    ...
    let tab = $state<'general' | 'stats' | 'attributes' | 'abilities' | 'preview'>('general');
...
<div
    class="fixed inset-0 z-[100] flex items-center justify-center bg-black/80 lg:p-4"
    onclick={handleBackdropClick}
    role="button"
    aria-label={$t('session.enemy_modal.close')}
    tabindex="-1"
    transition:fly={{ duration: 200, opacity: 1 }}
>
    <!-- On mobile: full screen. On desktop: rounded & constrained -->
    <div
        class="bg-slate-900 w-full h-full lg:rounded-xl lg:max-w-6xl lg:h-[90vh] border-0 lg:border border-slate-700 shadow-2xl overflow-hidden flex flex-col lg:flex-row"
        role="dialog"
        aria-modal="true"
    ...
    >
          ...
          <div class="w-full lg:w-1/2 flex flex-col h-full border-r border-slate-700">
                ...
                <div class="flex border-b border-slate-700 bg-slate-900/50 overflow-x-auto">
                    <button onclick={() => tab = 'general'} class="flex-1 py-3 px-4 text-sm font-bold border-b-2 whitespace-nowrap transition-colors {tab === 'general' ? 'border-indigo-500 text-indigo-400 bg-slate-800' : 'border-transparent text-slate-500 hover:text-white'}">{$t('session.enemy_modal.tab_general')}</button>
                    <button onclick={() => tab = 'stats'} class="flex-1 py-3 px-4 text-sm font-bold border-b-2 whitespace-nowrap transition-colors {tab === 'stats' ? 'border-indigo-500 text-indigo-400 bg-slate-800' : 'border-transparent text-slate-500 hover:text-white'}">{$t('session.enemy_modal.tab_stats')}</button>
                    <button onclick={() => tab = 'abilities'} class="flex-1 py-3 px-4 text-sm font-bold border-b-2 whitespace-nowrap transition-colors {tab === 'abilities' ? 'border-indigo-500 text-indigo-400 bg-slate-800' : 'border-transparent text-slate-500 hover:text-white'}">{$t('session.enemy_modal.tab_abilities')}</button>
                    <button onclick={() => tab = 'preview'} class="lg:hidden flex-1 py-3 px-4 text-sm font-bold border-b-2 whitespace-nowrap transition-colors {tab === 'preview' ? 'border-indigo-500 text-indigo-400 bg-slate-800' : 'border-transparent text-slate-500 hover:text-white'}">{$t('session.enemy_modal.preview')}</button>
                </div>
                ...
                <div class="flex-1 overflow-y-auto p-6 custom-scrollbar bg-slate-800/50">
                    ...
                    {:else if tab === 'stats'}
                        <div class="space-y-6 animate-in fade-in slide-in-from-bottom-2 duration-300">
                             ...
                             <div class="grid grid-cols-2 gap-4">
                                <div><label for="enemy-defense" class="text-xs text-slate-500 uppercase font-bold block mb-1">{$t('session.enemy_modal.defense')}</label><input id="enemy-defense" type="number" class="w-full bg-slate-900 border border-slate-700 rounded p-2 text-white" bind:value={form.defense} /></div>
                                <div><label for="enemy-health" class="text-xs text-slate-500 uppercase font-bold block mb-1">{$t('session.enemy_modal.health')}</label><input id="enemy-health" type="number" class="w-full bg-slate-900 border border-slate-700 rounded p-2 text-white" bind:value={form.health} /></div>
                             </div>

                             <div class="grid grid-cols-2 gap-4">
                                <div><label for="enemy-damage" class="text-xs text-slate-500 uppercase font-bold block mb-1">{$t('session.enemy_modal.damage')}</label><input id="enemy-damage" type="number" class="w-full bg-slate-900 border border-slate-700 rounded p-2 text-white" bind:value={form.damage} /></div>
                            ...
                            <!-- Attributes -->
                            <div>
                                <h4 class="text-indigo-400 font-bold text-sm uppercase mb-3 text-center border-b border-indigo-500/20 pb-2">{$t('session.enemy_modal.main_attributes')}</h4>
                                <div class="grid grid-cols-4 gap-3">
                                    {#each ['str', 'agi', 'int', 'wil'] as attr}
                                        <div class="bg-slate-900 p-2 rounded border border-slate-700 text-center">
                                            <label for="attr-{attr}" class="text-[10px] text-slate-500 uppercase font-bold block mb-1 text-ellipsis overflow-hidden whitespace-nowrap">{$t('sofww.attributes.' + attr).substring(0,3)}</label>
                    ...
                    {:else if tab === 'preview'}
                        <!-- Mobile Preview Tab -->
                         <div class="flex-1 flex items-start justify-center p-4 min-h-0 animate-in fade-in slide-in-from-right-2 duration-300">
                             {@render previewCard()}
                         </div>
                    {/if}
                </div>

                <div class="p-4 border-t border-slate-700 bg-slate-900 flex justify-end gap-3 safe-area-bottom">
